...
uses
  ...
  NxGrid, NxColumns, NxColumnClasses, Menus, StdCtrls, Commctrl, jpeg;
...
begin
 ...
 strSQL := 'SELECT distinct MCHANNEL FROM M_CHANNEL where MCH_ACTIVE = ''1'' ORDER BY MCHANNEL';
 RecSet(strSQL);
...
var
 i : integer;
 angka : string;
 // epgReplace, synEng, synInd : String;
begin
 if strUserACC = 'Admin' then
  begin
   for i := 0 to ngCA.RowCount-1 do
...
if (not (Key in ['0'..'9', #8])) then
  Key := #0;
  ShowBalloonTip(txtSearch, 1, 'Input CA PAckage', 'Please input CA Package Number Only here!', clBlue, clNavy);
end;
